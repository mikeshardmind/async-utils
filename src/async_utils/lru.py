...
class LFUNode(t.Generic[K, V]):
    __slots__ = ("freq", "key", "time", "val")
    ...
    def __repr__(self) -> str:
        return f"<LFUNode key={self.key!r} val={self.val!r} freq={self.freq:.7g}>)"


class DecayingLFU(t.Generic[K, V]):
    """Not Supported yet."""
    ...
    def __init__(self, maxsize: int) -> None:
        self._cache: dict[K, LFUNode[K, V]] = {}
        self._freqs: list[LFUNode[K, V]] = []
        self._maxsize: int = maxsize
        self._internal_lock: threading.RLock = threading.RLock()

    def decay(self, alpha: float, /) -> None:
        with self._internal_lock:
            for node in self._freqs:
                node.freq *= alpha
    ...
    def __contains__(self, *_dont_care: t.Any) -> t.Never:
        msg = "Do not check for existence prior to operation in an lfu cache"
        raise ContainsMisuse(msg)

    def __iter__(self) -> t.Never:
        msg = "Do not attempt to iterate over an lfu cache"
        raise IterationMisuse(msg)
    ...
    def __setitem__(self, key: K, value: V, /) -> None:
        with self._internal_lock:
            try:
                ...
                if len(self._cache) > self._maxsize:
                    rm = heapq.heapreplace(self._freqs, node)
                    self._cache.pop(rm.key, None)
                else:
                    heapq.heappush(self._freqs, node)
            else:
                node.inc()
                node.val = value
    ...
    def remove(self, key: K, /) -> None:
        with self._internal_lock:
            if node := self._cache.pop(key, None):
                self._freqs.remove(node)
                heapq.heapify(self._freqs)
